...
def run_struct_draw(pdb_file_path: str, pdb_file_type: str = None) -> None:
    ...
    canvas = Canvas('white')
    canvas.add_chain(Chain(chain_A, 'struct', shape_size=10, split=60))
    canvas.add_chain(Chain(chain_B, 'struct', shape_size=10, split=60))
    canvas.add_title('DejaVuSans.ttf', 10, 'Test Title', 'left')
